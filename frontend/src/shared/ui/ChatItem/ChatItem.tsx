    ...
    >
      <div className="flex items-center gap-4 min-w-0 flex-1">
        <img
          src={image}
          alt={name}
          className="w-15.5 h-15.5 rounded-full object-cover flex-shrink-0"
        />
        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
          <h3 className="font-semibold text-font-primary text-lg truncate">{name}</h3>
      ...
      <div className="flex flex-col justify-center items-center gap-2 flex-shrink-0">
        <span className="text-xs text-font-secondary whitespace-nowrap">{timestamp}</span>
        {unreadMessages > 0 && (
